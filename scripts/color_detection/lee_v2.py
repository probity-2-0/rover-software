...
path = r"C:\Root\Projects\probity\images\colorful_landscape.jpeg" #This image has more distinct colors, so it should work better for testing the color detection algorithm. I will use this image to see if the defined color boundaries can effectively isolate the red, green, and blue regions in the photo.
img = cv2.imread(path)
